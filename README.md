...
- **API Backend**: Flask backend with hello endpoint and math operations
...
**Error Cases:**
- `400` - Missing required fields (operation, a, or b)
- `400` - Unsupported operation type
- `400` - Invalid number values for a or b
- `400` - Division by zero (returns: `{"error": "Cannot divide by zero"}`)

**Notes:**
- Both integers and decimal numbers are supported
- Numbers can be provided as strings and will be converted to floats
...
- **Math Operations**: Support for all basic arithmetic with decimal precision
